...
const Layout06 = () => {
  ...
  const SearchForm = ({ value, setValue, selectedCity, setSelectedCity, cities }) => (
    ...
    <div class="h_formgroup-inline flex w-full">
      <div class="h_field">
        <label for="firstname5" class="p-sr-only">플랜트</label>
        <InputText value={value} onChange={(e) => setValue(e.target.value)}  placeholder="선택해주세요"/>  
      </div>
      <div class="h_field">
        <label for="firstname5" class="p-sr-only">요청기간</label>
        <IconField iconPosition="right">
            <InputIcon className="pi pi-search"> </InputIcon>
            <InputText placeholder="입력해주세요" />
        </IconField>
        <Button label="비교검색" text className="in-searchbtn"/> 
      </div>
        <div class="h_field">
        <label for="firstname5" class="p-sr-only">나라선택</label>
          <Dropdown value={selectedCity} onChange={(e) => setSelectedCity(e.value)} options={cities} optionLabel="name" 
            placeholder="선택해주세요"/>
        </div>
        <div class="h_field">
          <span className="spantext">검색조건</span>
